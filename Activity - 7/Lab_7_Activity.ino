...
// INBOUND SIGNALS (from Serial Monitor OR from Python/FastAPI):
// - "1" → toggle RED LED on/off
// - "2" → toggle GREEN LED on/off
// - "3" → toggle BLUE LED on/off
// - "0" → turn ALL LEDs OFF
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\n' || c == '\r') {
      ...
      if (idx < sizeof(inputBuf) - 1) {
        ...
        // Buffer overflow: reset buffer to prevent getting stuck
        idx        = 0;
        inputBuf[0] = '\0';
      }
    }
